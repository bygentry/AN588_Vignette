...
-   Eigenvectors provide the "direction" within the scatter plot
...
# this will produce a distribution that has higher genotype A frequency
...
For an example where we only consider two classes, lets start by calculating the values we need. The means are easy, but we have to make sure to distinguish between genotype AND gene.
...
grid$pred_geno <- predict(lda_model, newdata = grid)$geno
...
ggplot(lda_data, aes(x = LD1, y = LD2, color = geno)) +
  ...
  geom_contour(data = grid, aes(x = LD1, y = LD2, z = as.numeric(pred_geno)),
               color = "black", bins = 1) +  
  coord_fixed() +
  theme_bw()
```
