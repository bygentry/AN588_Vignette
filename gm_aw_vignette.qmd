...
pairs.panels(tidy.data[, c(5, 8:10)],
             gap = 1,
             bg = (colorRampPalette(c("blue", "red"))(161)[d$Species]),
             pch = 21,
             hist.col = "lightpink",
             hist.border = "white")
```
...
In order to predict the species of a primate based on these variables, we'll need to create a training data set through random sampling. We'll use 66% of the set for training and the other 34% for testing:
