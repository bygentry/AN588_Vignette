...
# Because LDA assumes that the data all share the same covariance matrix, we can't just use the standard deviations of the raw data.

# This is computed internally by lda(), but you can compute it manually like this:
cov_pooled <- cov(lda_data[lda_data$response == "A", 1:2]) * 
              (sum(lda_data$response == "A") - 1) +
              cov(lda_data[lda_data$response == "B", 1:2]) * 
              (sum(lda_data$response == "B") - 1)

cov_pooled <- cov_pooled / (nrow(lda_data) - 2)
cov_pooled
